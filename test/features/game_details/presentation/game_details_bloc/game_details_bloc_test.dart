...
void main() {
  ...
  setUp(() {
    ...
    sut = GameDetailsBloc(repository: gameDetailsRepository);
  });
  ...
  final GameDetailsModel testModel = GameDetailsModel(
    ...
    userRating: UserRatingModel(exceptional: 1, recommended: 1, meh: 1, skip: 1),
  );

  blocTest<GameDetailsBloc, GameDetailsState>(
    'emits [GameDetailsLoading, GameDetailsSuccess] when GameDetailsRequested is added.',
    build: () {
      when(() => gameDetailsRepository.getGameDetails(gameId: 1))
          .thenAnswer((_) async => testModel);
      return sut;
    },
    act: (bloc) => bloc.add(const GameDetailsRequested(gameId: 1)),
    expect: () => [
  ...
  group('Error handling tests', () {
    blocTest<GameDetailsBloc, GameDetailsState>(
      'emits [GameDetailsLoading, GameDetailsError] when getGameDetails throws exception',
      build: () {
        when(() => gameDetailsRepository.getGameDetails(gameId: 1)).thenThrow(
          Exception('Failed to load game details'),
        );
        return sut;
      },
      act: (bloc) => bloc.add(const GameDetailsRequested(gameId: 1)),
